...
- (IBAction)SendOnClick:(id)sender {
    ...
    //wangyao 0413
    if ([self.SendButton.titleLabel.text isEqualToString:@"快速回复"]  ) {
        [StatusTool statusToolPostReplyWithReplyText:@"111" communityID:self.post_item.belong_community_id forumID:self.post_item.belong_forum_id postID:self.post_item.post_id userID:self.UserID Success:^(id object) {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提交回复" message:@"回复成功" delegate:self cancelButtonTitle:@"好" otherButtonTitles:nil, nil];
            alert.delegate = self;
            [alert show];
        } failurs:^(NSError *error) {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提交回复" message:@"回复失败" delegate:self cancelButtonTitle:@"退出" otherButtonTitles:nil, nil];
                        alert.delegate = self;
                        [alert show];
        }];

    }else if([self.SendButton.titleLabel.text isEqualToString:@"查看回复"]  ){
         ReplyListViewController *re = [ ReplyListViewController createFromStoryboardName:@"ReplyList" withIdentifier:@"reply"];
        re.post_id = self.post_item.post_id ;
